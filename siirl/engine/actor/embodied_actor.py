...
class RobDataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: TensorDict):
        ...
        old_log_probs_data = data['old_log_probs']
        ...
        # advantages 检查
        logger.info(f"[DEBUG UPDATE] advantages stats: {debug_tensor_stats('advantages', data['advantages'])}")
        
        # 数据一致性检查
        logger.info(f"[DEBUG UPDATE] === 数据一致性检查 ===")
        logger.info(f"[DEBUG UPDATE] responses shape: {data['responses'].shape}")
        logger.info(f"[DEBUG UPDATE] input_ids shape: {data['input_ids'].shape}")
        logger.info(f"[DEBUG UPDATE] old_log_probs shape: {old_log_probs_data.shape}")
        logger.info(f"[DEBUG UPDATE] advantages shape: {data['advantages'].shape}")
        if 'finish_step' in data:
            logger.info(f"[DEBUG UPDATE] finish_step shape: {data['finish_step'].shape}, mean: {data['finish_step'].float().mean().item():.2f}, min: {data['finish_step'].min().item()}, max: {data['finish_step'].max().item()}")
        
        # 验证 shape 一致性
        batch_size = data['responses'].size(0)
        expected_response_len = data['responses'].size(1) * data['responses'].size(2)
