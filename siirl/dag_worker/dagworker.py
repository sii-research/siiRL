...
from siirl.data_coordinator import preprocess_dataloader, Samples2Dict, Dict2Samples, SampleManager, MetaInfo
                            ...
                            if whether_put_data(self._rank, is_current_last_pp_tp_rank0, next_dp_size, cur_dp_size, cur_node, next_node):
                                with timer(self.enable_perf, "put_data_to_buffer", timing_raw):
                                    if self._multi_agent and next_node.node_role == NodeRole.ADVANTAGE:
                                        ...
                                        ## add for develop test
                                        if cur_node.node_role == NodeRole.REWARD and self.config.algorithm.adv_estimator == AdvantageEstimator.GAE:
                                            sample_managers = Dict2Samples(data=node_output.batch)
                                            node_output.batch = Samples2Dict(sample_managers)
                                        if cur_node.node_role == NodeRole.ADVANTAGE and self.config.algorithm.adv_estimator == AdvantageEstimator.GRPO:
                                            sample_managers = Dict2Samples(data=node_output.batch)
                                            node_output.batch = Samples2Dict(sample_managers)
...
# ==========================================================================================
    ...
    def put_data_to_buffers(
        self, key: str, 
        data: Optional[Union[TensorDict, List[SampleManager]]],
        source_dp_size: int, 
        dest_dp_size: int, 
        enforce_buffer: bool, 
        ...
        try:
            ...
            data["padding_values"] = NonTensorData({"input_ids": self.validate_tokenizer.pad_token_id, "responses": self.validate_tokenizer.pad_token_id, "labels": -100, "attention_mask": 0, "response_mask": 0})
            data["padding_side"] = NonTensorData(self.validate_tokenizer.padding_side)
