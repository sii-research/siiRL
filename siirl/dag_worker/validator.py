...
class Validator:
    ...
    def validate(self, global_step: int) -> Dict[str, float]:
        ...
        for i in range(self.dataloader.num_val_batches):
            ...
            with timer(self.enable_perf, "prep_and_generate", self.val_timedict):
                ...
                val_batch = test_batch_proto.repeat(n_samples, interleave=True)
    ...
    def _generate_for_validation(self, batch_proto: DataProto) -> DataProto:
        ...
        if self.multi_agent is False:
            if self.rollout_mode == 'sync':
                ...
                output = loop.run_until_complete(self.async_rollout_manager.generate_sequences(gen_batch))
        else:
    ...
    def _score_and_package_results(self, generated_proto: DataProto) -> List[ValidationResult]:
        ...
        if self.rollout_mode == 'async' and self.async_rollout_manager is None:
            return []
        if self.multi_agent and 'responses' not in generated_proto.batch:
            return []
