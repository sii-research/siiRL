...
.. list-table::
   ...
   * - ``vjepa_embedding``
     - ``(B*N, embed_dim)``
     - V-JEPA visual features


Embodied Rollout: VLA-Environment Interaction Loop
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Embodied Rollout Data Flow Diagram
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Key Configuration Parameters for Rollout
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. list-table::
   ...
   * - ``max_steps``
     ...
     - Maximum steps per episode (default: 512)
   * - ``action_chunks_len``
     - ``actor_rollout_ref.embodied.action_chunks_len``
     - Actions per VLA forward pass (default: 8)
   * - ``action_token_len``
     - ``actor_rollout_ref.embodied.action_token_len``
     - Action dimensions (default: 7 for xyz+quat+gripper)
   * - ``temperature``
     - ``actor_rollout_ref.rollout.temperature``
     - Sampling temperature for action generation
   * - ``do_sample``
     - ``actor_rollout_ref.rollout.do_sample``
     - Enable stochastic sampling (True for training)


Data Filtering (Embodied Sampling)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Reward Computation (VJEPA-based)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.. list-table::
   ...
   * - **Embodied Rollout**
     - ``siirl/workers/rollout/embodied_rollout.py``
   * - Environment Adapter
     - ``siirl/workers/environment/embodied.py``
   * - V-JEPA Embedding
     - ``siirl/utils/embodied/video_emb.py``
   * - Data Filtering
     - ``siirl/algorithm/embodied/sampling.py``
   * - VJEPA Reward
     - ``siirl/utils/reward_score/embodied.py``
   * - GRPO Advantage
     - ``siirl/workers/dag_worker/core_algos.py``
   * - VLA Actor
